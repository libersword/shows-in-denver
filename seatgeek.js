...
function artistsLike(performerID) {
  var baseURL =
    "https://api.seatgeek.com/2/recommendations?performers.id=" +
    performerID +
    "&venue.city=denver&datetime_utc.gte=" +
    startDate +
    "&datetime_utc.lte=+" +
    endDate +
    "&client_id=" +
  ...
  $.ajax({
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      $.each(results[i].event.performers, function(j) {
        ...
        artistImg = results[i].event.performers[j].image;
        if (artistImg === null) {
          artistImg = "https://picsum.photos/300/200";
        }
      });
      ...
      createCard(artistName, artistImg, eventDate, venue, lat, lon);
    }
  });
}
